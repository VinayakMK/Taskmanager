        ...
        <div className="tasks">
            {
                tasks.sort((a)=>a.isComplete ? 1 : -1).map(
                    (task, index)=><div key={index} className={+(task.isComplete ? "completed": "incomplete")}>
                        <input type="checkbox" checked={task.isComplete} onChange={()=>markCompleted(index)} className="edit-input"/>
                        {
                            task.isEditing ?
                            
                            <input value={task.content} onChange={(event)=>updateValue(index, event.target.value)}/>
                            :
